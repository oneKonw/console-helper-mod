 ...
 * @LastEditors: hyt
 * @LastEditTime: 2023-03-14 19:14:01
...
const switchFileName = (selectFileName, fileName) => {
  switch (selectFileName) {
    case '打印文件名':
      return winPath(fileName.match(/[^\\/]+(?=\.)/g)[0]);
    case '打印文件名+文件后缀名':
      return winPath(fileName.replace(/^.*[\\\/]/, ''));
    case '打印文件名+后缀名+上级目录':
      const regex = /[^/\\]+/g;
      const matches = fileName.match(regex);
      return winPath(matches[matches.length - 2] + "/" + fileName.replace(/^.*[\\\/]/, ''));
    case '打印完整路径':
